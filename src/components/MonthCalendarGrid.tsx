          ...
          <table className="w-full border-collapse" style={{ tableLayout: "fixed", minWidth: "max-content" }}>
            <thead>
              <tr>
                <th
                  rowSpan={3}
                  className="w-16 sm:w-24 lg:w-36 border-b border-r border-border bg-card align-top p-2 sm:p-3 text-left"
                >
              ...
              <tr>
                {days.map((d) => (
                  <th
                    key={dateKey(d.date)}
                    className="text-[8px] sm:text-[10px] font-medium text-muted-foreground border-b border-r border-border first:border-l px-0.5 py-1 sm:py-2 w-6 min-w-6 sm:w-7 sm:min-w-7 text-center"
                  >
              ...
              <tr>
                {days.map((d) => {
                  ...
                  return (
                    <th
                      key={dateKey(d.date)}
                      className={`text-[8px] sm:text-[10px] font-medium border-b border-r border-border first:border-l px-0.5 py-0.5 sm:py-1 w-6 min-w-6 sm:w-7 sm:min-w-7 text-center ${
                        isCurrentDay
            ...
            <tbody>
              {habits.map((h) => (
                <tr key={h.id}>
                  <td className="border-b border-r border-border px-2 sm:px-3 py-0.5 h-6 sm:h-7 align-middle text-[10px] sm:text-xs text-foreground truncate bg-card w-16 sm:w-24 lg:w-36">
                    {h.name}
                  </td>
                  {days.map((d, dayIndex) => {
                    ...
                    return (
                      <td
                        key={key}
                        className={`border-b border-r border-border p-0.5 w-6 min-w-6 sm:w-7 sm:min-w-7 h-6 sm:h-7 align-middle ${dayIndex === 0 ? "border-l" : ""}`}
                      >
                        <button
                          type="button"
                          ...
                          className={`w-full h-5 sm:h-6 flex items-center justify-center rounded border transition-colors ${
                            lightTheme
                              ...
                              : completed
                                ? "bg-success text-primary-foreground border-border"
